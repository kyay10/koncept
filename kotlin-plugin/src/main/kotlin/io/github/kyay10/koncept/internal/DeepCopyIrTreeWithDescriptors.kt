...
import io.github.kyay10.koncept.utils.mapOrOriginal
...
internal class DeepCopyIrTreeWithSymbolsForInliner(
  ...
  private fun generateCopyName(name: Name) =
    if (name.isSpecial) name else Name.identifier(name.toString() + "_" + (nameIndex++).toString())
  ...
  private inner class InlinerTypeRemapper(
    ...
    private fun remapTypeArguments(
      ...
      arguments.forEachIndexed { index, argument ->
        if (argument is IrTypeProjection) {
          ...
          newArguments!![index] = remapTypeAndOptionallyErase(argument.type, erasedParameters)?.let { newType ->
    ...
    private fun remapTypeAndOptionallyErase(
      type: IrType,
      erasedParameters: MutableSet<IrTypeParameterSymbol>?
    ): IrType? = DeepRecursiveFunction<IrType, IrType?> { type ->
      if (type !is IrSimpleType) return@DeepRecursiveFunction type
      ...
      if (erasedParameters != null && substitutedType != null && (classifier as? IrTypeParameterSymbol)?.owner?.isReified == false) {
        ...
        val erasedUpperBound = callRecursive(upperBound)
          ?: error("Cannot erase upperbound ${upperBound.render()}")

        erasedParameters.remove(classifier)

        return@DeepRecursiveFunction erasedUpperBound.mergeNullability(type)
      }

      if (substitutedType is IrDynamicType) return@DeepRecursiveFunction substitutedType

      if (substitutedType is IrSimpleType) {
        return@DeepRecursiveFunction IrSimpleTypeImpl(
          substitutedType.classifier,
          type.isMarkedNullable() or substitutedType.isMarkedNullable(),
