...
fun <T : Any> Iterable<IndexedValue<T?>>.filterNotNullIndices(): List<IndexedValue<T>> {
...
inline fun <reified T> Iterable<*>.firstIsInstance(predicate: (T) -> Boolean): T {
  for (element in this) {
...
inline fun <reified T : Any> Iterable<*>.firstIsInstanceOrNull(predicate: (T) -> Boolean): T? {
  for (element in this) {
    if (element is T && predicate(element)) {
      return element
    }
...
inline fun <reified T> List<*>.lastIsInstance(predicate: (T) -> Boolean): T {
  val iterator = this.listIterator(size)
  while (iterator.hasPrevious()) {
    val element = iterator.previous()
    if (element is T && predicate(element)) return element
  }
  throw NoSuchElementException("List contains no element matching the predicate.")
}

inline fun <reified T : Any> List<*>.lastIsInstanceOrNull(predicate: (T) -> Boolean): T? {
  val iterator = this.listIterator(size)
  while (iterator.hasPrevious()) {
    val element = iterator.previous()
    if (element is T && predicate(element)) return element
